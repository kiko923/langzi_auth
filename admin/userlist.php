...
<option value="2">合作商权限</option>
<option value="3">授权商权限</option>
<option value="0">封禁该用户</option>
</select>
</div><br/>
...
<option value="1">超级管理员</option><option value="2">合作商权限</option><option value="3">授权商权限</option><option value="0">封禁该用户</option>
</select>
</div><br/>
...
<?php
	...
	if($pass==$user)exit("<script type='text/javascript'>layer.alert('密码不能跟账号一样！',{icon:5,closeBtn:0},function(){history.go(-1)});</script>");
	...
	if($DB->query("INSERT INTO `auth_user`(`user`, `pass`, `qq`, `type`, `addtime`, `status`) VALUES ('".$user."','".md5($pass)."','".$qq."','".$type."','".$date."','".$status."')")){
	...
	if(isset($_POST['user']) && isset($_POST['pass']) && isset($_POST['type'])){
		...
		if($DB->query("update auth_user set user='{$user}',pass='{$pass}',type='{$type}',status='{$status}' where uid='{$id}'")){
	...
	if($mod=='search'){
		...
		$numrows=$DB->count("SELECT count(*) from auth_user WHERE{$sql}");
		...
		$numrows=$DB->count("SELECT count(*) from auth_user");
		$sql=" 1";
...
<div class="modal-body">
<form action="./userlist.php" method="GET">
<select name="type" class="form-control"><option value="1">超级管理员</option><option value="2">合作商权限</option><option value="3">授权商权限</option></select><br />
<input type="submit" class="btn btn-primary btn-block" value="搜索"></form>
</div>
<div class="modal-footer">
<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
...
$rs=$DB->query("SELECT * FROM auth_user WHERE{$sql} order by uid desc limit $pageu,$pagesize");
