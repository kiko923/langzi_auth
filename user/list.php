...
<?php
	...
	if(isset($_GET['search'])){
		$search=daddslashes(base64_decode($_GET['search']));
		if($_GET['type']==1)
			$sql=($_GET['method']==1)?" `qq` LIKE '%{$search}%'":" `qq`='{$search}'";
		elseif($_GET['type']==2)
			$sql=($_GET['method']==1)?" `url` LIKE '%{$search}%'":" `url`='{$search}'";
		elseif($_GET['type']==3)
			$sql=($_GET['method']==1)?" `authcode` LIKE '%{$search}%'":" `authcode`='{$search}'";
		elseif($_GET['type']==4)
			$sql=($_GET['method']==1)?" `sign` LIKE '%{$search}%'":" `sign`='{$search}'";
		else{
			if(is_numeric($search))$column='qq';
			elseif(strpos($search,'.')!==false)$column='url';
			else $column='authcode';
			$sql=($_GET['method']==1)?" `{$column}` LIKE '%{$search}%'":" `{$column}`='{$search}'";
		}
		$numrows=$DB->count("SELECT count(*) from auth_site WHERE{$sql} and daili='{$userrow['uid']}'");
		$con='包含 '.$search.' 的共有 <b>'.$numrows.'</b> 个域名';
		$link='&search='.$_GET['search']."&method=".$_GET['method'];
	}elseif(isset($_GET['qq'])){
		$qq=daddslashes($_GET['qq']);
		$sql=" `qq`='{$qq}'";
		$numrows=$DB->count("SELECT count(*) from auth_site WHERE{$sql} and daili='{$userrow['uid']}'");
		$con='QQ '.$_GET['qq'].' 共有 <b>'.$numrows.'</b> 个域名';
		$link='&qq='.$_GET['qq'];
	}else{
		$numrows=$DB->count("SELECT count(*) from auth_site WHERE daili='{$userrow['uid']}'");
		$sql=" 1";
		$con='授权平台代理用户(UID:'.$userrow['uid'].')共有 <b>'.$numrows.'</b> 个域名';
	}
?>
...
<thead><tr><th>ID</th><th>QQ</th><th>域名</th><th>时间</th><th>状态</th><th style="width: 200px;" class="text-center">操作</th></tr></thead>
<tbody>
<?php
$rs=$DB->query("SELECT * FROM auth_site WHERE{$sql} and daili='{$userrow['uid']}' order by id desc limit $pageu,$pagesize");
while($res = $DB->fetch($rs)){
	if($res['active']==0){
