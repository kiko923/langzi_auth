...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">浪子授权系统</h3>
	</div>
...
?>";
deldir("../includes/download");
mkdir("../includes/download",0755);
mkdir("../includes/download/install",0755);
mkdir("../includes/download/update",0755);
file_put_contents("../includes/download/authcode.php",$hex);
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from auth_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过浪子授权系统</div>
...
define('PACKAGE_SUFFIX','".random(5)."'); //下载目录后缀
?>";
...
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
	...
	require './db.class.php';
	$sql=file_get_contents("mulin.sql");
	$sql=explode(';',$sql);
...
<input type="text" class="form-control" name="title" value="浪子授权系统">
<label for="name">站长ＱＱ:</label>
<input type="text" class="form-control" name="kfqq" value="1931135643">
<br><input type="submit" class="btn btn-primary btn-block" name="submit" value="保存配置">
</form>
</div>
</div>
...
if($user==NULL or $pass==NULL or $title==NULL or $kfqq==NULL){
	echo '<div class="alert alert-danger">保存错误,请确保每项都不为空<hr/><a href="javascript:history.back(-1)"><< 返回上一页</a></div>';
}else{
	require_once "core.php";
	rename("../includes/download/install","../includes/download/install_".PACKAGE_SUFFIX);
	rename("../includes/download/update","../includes/download/update_".PACKAGE_SUFFIX);
	saveSetting('admin_user',$user);
	saveSetting('admin_pass',md5($pass));
	saveSetting('title',$title);
	saveSetting('kfqq',$kfqq);
	saveSetting('content','您的网站未授权！购买正版请联系QQ：'.$kfqq);
